...
module.exports = {
  ...
  version: "1.0",
  description:
    "Adaptive regime, volatility scaling, profit lock, risk spread, and emergency brake.",
  ...
  checkDrawdownBrake(portfolio, config) {
    ...
    const drawdown =
      this._portfolioATH > 0
        ? (this._portfolioATH - totalValue) / this._portfolioATH
        : 0;
    if (drawdown >= ddBrake) {
      if (!this._drawdownTriggered) {
        this._drawdownTriggered = true;
        console.log(
          `🛑 EMERGENCY BRAKE: Portfolio drawdown -${(drawdown * 100).toFixed(
            1
          )}%. All buys paused, profits locked.`
        );
      }
      return true;
  ...
  getTradeDecision({
    ...
    if (
      portfolio &&
      portfolio.currentDayBuys &&
      portfolio.currentDayBuys[symbol]
    ) {
      const spent = portfolio.currentDayBuys[symbol];
      if (spent / (portfolio.unlockedCash || 1) > spread) {
        if (process.env.DEBUG) {
          console.log(
            `[DEBUG][${symbol}] Risk spread limit reached for this coin.`
          );
        }
        return null;
      }
    ...
    const priList = (process.env.PRIORITY_CRYPTOS || "")
      .split(",")
    ...
    if (
      regime === "uptrend" &&
      isPriority &&
      price > costBasis * 1.01 &&
      price - lastPrice > atr * buyThresh
    ) {
  ...
  updateStrategyState(symbol, state, config) {
    ...
    if (state.priceHistory.length >= 2) {
      ...
      // Up ticks logic...
      if (curr > last) state.upTicks = (state.upTicks || 0) + 1;
      else state.upTicks = 0;
    } else {
      state.upTicks = 0;
    }
    // Always set trend to regime
