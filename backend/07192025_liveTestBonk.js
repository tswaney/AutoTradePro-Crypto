...
async function placeOrder(side, qty) {
  ...
  // Sign it
  const canonicalBody = stringify(body);
  const signature = signRequest(
    PUBLIC_API_KEY,
    timestamp,
    path,
    "POST",
    canonicalBody // pass the canonical string instead of the object
  );
  ...
  "x-api-key":     process.env.ROBINHOOD_API_KEY, // <-- Use the API key
  "x-public-key":  process.env.PUBLIC_API_KEY,    // <-- Use your public key
