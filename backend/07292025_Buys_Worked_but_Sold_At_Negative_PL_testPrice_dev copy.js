...
async function runStrategyForSymbol(symbol) {
  ...
  if (action === 'BUY') {
    ...
    if (spend > portfolio.cashReserve) {
      console.log(`⚠️  BUY skipped for ${symbol}: Not enough cash (need $${spend.toFixed(2)}, have $${portfolio.cashReserve.toFixed(2)})`);
      return;
    }
    const actualQty = spend / info.price;
    if (actualQty < config.minTradeAmount) {
      console.log(`⚠️  BUY skipped for ${symbol}: actualQty (${actualQty}) < minTradeAmount`);
      return;
    }
